...
class Counting(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if (
            ...
            in self.data[str(message.guild.id)]["countingChannels"]
        ):
            ...
            if string.isdigit():
                ...
                if not (
                    num
                    == dict2["countingChannels"][message.channel.name.lower()]["count"]
                    + 1
                ) or (
                    message.author.id
                    ...
                    if (
                        ...
                        self.data[str(message.guild.id)] = dict2
                        await message.add_reaction("✅")
                self.db.collection("servers").document(str(message.guild.id)).set(dict2)

    @check.is_admin()
    @commands.command(
        name="addchannel",
        description="Adds a counting channel, from which counts will be recorded :D",
        usage="addchannel <channel name (empty if referring to current channel)>",
        aliases=["addc"],
    )
    async def add_counting_channel(self, ctx, name: str = None):
        ...
        self.cooldown[str(ctx.guild.id)][ctx.channel.name.lower()] = time.time() - 5
        self.db.collection("servers").document(str(ctx.guild.id)).set(dict1)
    ...
    @commands.command(
        name="Set Count",
        aliases=["setcount"],
        description="Sets the current count in the current channel.",
        usage="setcount <number>",
        hidden=True,
    )
    async def setcurrentcount(self, ctx, num: int):
        ...
        self.db.collection("servers").document(str(ctx.guild.id)).set(dict2)
    ...
    @commands.command(
        ...
        usage="countingchannel",
        aliases=["csi", "cs", "countingchannel"],
    )
    ...
    async def countinginfo(self, ctx):
        ...
        for i in self.data[str(ctx.guild.id)]["countingChannels"]:
            leaderboard[i] = [
                self.data[str(ctx.guild.id)]["countingChannels"][i]["count"],
                self.data[str(ctx.guild.id)]["countingChannels"][i]["highest_count"],
            ]
        count = 0
        description = ""
